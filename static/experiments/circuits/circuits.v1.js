...
let displaySpeed = false; //false = slow/3fps, true = fast/24fps
let fr = 24;
...
function mousePressed() {
  let temp = circles.length;
  circles[circles.length] = new Circle(Math.ceil(mouseX/tileSize-1),Math.ceil(mouseY/tileSize)-1);
  circles[temp].show();
  numCircles++;
  rect(mouseX,mouseY, tileSize, tileSize);
  loop();
